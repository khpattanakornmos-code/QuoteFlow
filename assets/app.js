...
var READ_ACTIONS = [
  ...
  'getFinance','getPayroll','getReceivables','getDashboard','getReport','getUsers',
];
...
function canEdit(){
  var u=getUser();
  return !!(u && (u.canEdit===true || u.role==='admin' || u.role==='editor'));
}
function canManageUsers(){
  var u=getUser();
  return !!(u && (u.canManageUsers===true || u.role==='admin'));
}
function applyPermissions(){
  if(canEdit()) return;
  var selectors=[
    'button[onclick*="openCreate"]',
    'button[onclick*="QuoteFlow.openModal"]',
    'button[onclick*="addItemRow"]',
    'button[onclick*="saveQuotation"]',
    'button[onclick*="saveFin"]',
    'button[onclick*="savePay"]',
    'button[onclick*="saveRv"]',
    'button[onclick*="saveTimeline"]',
    'button[onclick*="saveReceivable"]',
    'button[onclick*="confirmActivate"]',
    'button[onclick*="updatePay"]',
    'button[onclick*="markPaid"]',
    'button[onclick*="delFin"]',
  ...
  selectors.forEach(function(sel){
    document.querySelectorAll(sel).forEach(function(el){
      if(el.dataset.permissionKeep==='true') return;
      el.disabled=true;
      el.classList.add('permission-disabled');
      el.title='บัญชีนี้มีสิทธิ์ดูข้อมูลเท่านั้น';
    });
  });
}
function logout(){
  ...
  google.accounts.id.initialize({
    ...
    callback:function(resp){
      ...
      api('login',{email:p.email,name:p.name||p.email.split('@')[0]}).then(function(r){
        var u=(r&&r.success&&r.user)?r.user:{email:p.email,name:p.name||p.email.split('@')[0],role:'viewer',canViewAll:true,canEdit:false,canManageUsers:false};
...
var COMPANY = {
  ...
  signature : 'data:image/svg+xml;utf8,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 430"><rect width="900" height="430" fill="none"/><g fill="none" stroke="#171735" stroke-width="20" stroke-linecap="round" stroke-linejoin="round"><path d="M73 166 C140 83 257 42 392 54 C443 58 475 64 506 80"/><path d="M74 166 C164 164 268 145 404 127 C476 117 541 103 604 88 C622 84 639 91 632 104 C594 166 532 226 455 277"/><path d="M323 287 C343 246 391 233 435 246 C468 256 485 283 473 313 C460 347 414 351 391 323 C370 298 391 267 429 244"/><path d="M275 397 C286 329 315 294 365 281 C421 266 470 286 506 322"/><path d="M513 293 C552 257 591 262 613 293 C632 318 617 350 585 348 C557 346 552 321 575 303 C608 279 650 276 690 289"/><path d="M685 290 C705 252 747 253 758 292 C767 326 753 348 732 345"/><path d="M756 289 C789 255 827 259 834 294 C840 324 824 349 797 346"/><path d="M834 292 C852 260 874 247 895 238"/></g></svg>'),
    ...
    '<table width="100%" cellpadding="0" cellspacing="0" style="margin-top:14px;border-top:1.5px solid ' + B + ';" class="no-break">' +
      '<tr>' +
        ...
        '<td width="45%" style="padding:20px 22px;text-align:center;vertical-align:bottom;">' +
          '<div style="font-size:13px;color:#666;margin-bottom:6px;">ผู้เสนอราคา</div>' +
          '<div style="height:88px;display:flex;align-items:flex-end;justify-content:center;margin-bottom:4px;">' +
            '<img src="' + C.signature + '" style="display:block;width:240px;max-width:100%;max-height:88px;object-fit:contain;">' +
          '</div>' +
...
window.QuoteFlow={
  ...
  canEdit,canManageUsers,applyPermissions,
